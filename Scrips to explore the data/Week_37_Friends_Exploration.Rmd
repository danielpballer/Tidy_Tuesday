---
title: "Tidy_Tuesday_Week37_Friends_Exploration"
...
write_csv(friends_emotions, "friends_emotions.csv")
write_csv(friends_info, "friends_info.csv")
```
...
friends_info %>% 
  ...
  ggplot()+
      ...
      geom_curve(aes(x = 190, y = 17.5, xend = 169, yend = 15.5),
                 arrow = arrow(length = unit(0.07, "inch")), size = 0.5,
...
friends_info %>% filter(us_views_millions>40) %>% select(title)
friends_info %>% filter(us_views_millions<18) %>% select(title)
friends_info %>% filter(us_views_millions<18)
```
